...
function HistoricoVT_VR() {
  ...
  async function fetchHistorico() {
    try {
      ...
      // Como agora é manual, não precisamos buscar as faltas e presenças
      setHistorico(data || []);
    } catch (err: any) {
  ...
  const handlePrevisaoAutomatica = async () => {
    try {
      ...
      // 2. Buscar dados da frequência do mês
      ...
      const newEntries = activeProfiles.map(profile => {
        ...
        datesList.forEach((dateObj) => {
          ...
          if (isWorkedStatus) count++;
        });

        return {
          id: Date.now() + Math.random(),
          employee_id: profile.id,
          dias_previstos: count.toString(),
          dias_desconto: ""
        };
      });
  ...
  filteredHistorico.forEach(h => {
    const { vtEsperado, vrEsperado } = getExpectedValues(h.profile, h.dias_previstos, h.dias_desconto);
      ...
      <div className="frequencia-container">
        ...
        <div className="freq-annual-summary-wrapper" style={{ margin: "20px auto", maxWidth: "100%", padding: "0 20px" }}>
          {loading ? (
            ...
            <>
              <div style={{ display: "flex", gap: "20px", alignItems: "center", justifyContent: "flex-end", marginBottom: "16px" }}>
              ...
              <table className="freq-table" style={{ minWidth: "900px" }}>
                <thead>
                  <tr>
                    ...
                    <th rowSpan={2} style={{ textAlign: "center", verticalAlign: "middle", borderRight: "2px solid #e2e8f0" }}>Dias<br/>Previstos</th>
